...
public class User {
    ...
    public void createMeeting(String meetingID, String title, Date date, Time startTime, Time endTime, Room room){
        ...
        for (Meeting m : this.schedule.getMeetings()){
            if(date.getYear() == m.getDate().getYear() && date.getMonth() == m.getDate().getMonth()
                    && date.getDate() == m.getDate().getDate()
                    && (startTime.compareTo(m.getStartTime()) >= 0
                    && startTime.compareTo(m.getEndTime()) <= 0
                    || endTime.compareTo(m.getStartTime()) >= 0
                    && endTime.compareTo(m.getEndTime()) <= 0 )){
    ...
    public void acceptMeeting(Meeting meeting){
        if (this.meetingInvitations.contains(meeting)){
            boolean meetInTheSameInterval = false;
            for (Meeting m : this.schedule.getMeetings()){
                if(meeting.getDate().getYear() == m.getDate().getYear() && meeting.getDate().getMonth() == m.getDate().getMonth()
                        && meeting.getDate().getDate() == m.getDate().getDate()
                        && (meeting.getStartTime().compareTo(m.getStartTime()) >= 0
                        && meeting.getStartTime().compareTo(m.getEndTime()) <= 0
                        || meeting.getEndTime().compareTo(m.getStartTime()) >= 0
                        && meeting.getEndTime().compareTo(m.getEndTime()) <= 0 )){
                    meetInTheSameInterval = true;
                    break;
                }
            }
    ...
    public void inviteParticipant(Meeting meeting, User participant) {
        if(this.meetings.contains(meeting)) {
            Notification notification = new Notification(meeting.getMeetingID() + participant.getUserID() + 'a',
                    meeting, "You were invited to this meeting", participant);
            notification.send();
